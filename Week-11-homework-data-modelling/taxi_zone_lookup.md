...
The model follows a Dimensional Modeling (Star Schema) approach:

Center: The Fact_Taxi_Trips entity sits at the center, holding the business performance metrics.

Surroundings: The Dimension entities (Dim_Date, Dim_Vendor, Dim_RateCode, Dim_Payment, Dim_Location) surround the fact table.

Connections: All dimensions are directly connected to the central Fact table via Foreign Keys (Surrogate Keys). The Dim_Location entity has two distinct relationships with the Fact table to handle both Pickup and Dropoff contexts independently.
